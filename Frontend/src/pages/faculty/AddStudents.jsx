...
import { CheckCircle2, FileSpreadsheet, UserPlus, Pencil, Trash2, X, Check } from 'lucide-react'
import { PageLoader } from '@/components/ui/loader'
import Pagination from '@/components/ui/Pagination'
import { TabBar, SearchInput } from '@/components/shared'
import { useStudents, useCreateStudent, useBulkCreateStudents, useUpdateStudent, useDeleteStudent } from '@/hooks/useApi'
import { PAGE_SIZE } from '@/lib/constants'

const empty = { name: '', email: '', rollNumber: '', department: '', semester: '', batch: '' }
...
export default function FacultyAddStudents() {
  ...
  const submitCsv = async () => {
    const students = csvPreview.map(r => ({
      ...
      department: r.department, semester: r.semester,
      batch: r.batch,
    }))
    try {
    ...
    <div className="max-w-4xl mx-auto space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Students receive login credentials via email automatically</p>
        </div>
          ...
          <form onSubmit={submitManual} className="px-5 py-5 space-y-4">
            <div className="grid sm:grid-cols-2 gap-4">
              {[
                ...
                { k: 'department', label: 'Department',  ph: 'Computer Engineering', req: true },
              ...
              <div className="space-y-1.5">
                ...
                <Input type="number" min="1" max="8" className="h-9 text-sm border-border" placeholder="5"
                  value={form.semester} onChange={e => set('semester', e.target.value)} required />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs text-muted-foreground uppercase tracking-wide">Batch</Label>
                <Input className="h-9 text-sm border-border" placeholder="2022" value={form.batch} onChange={e => set('batch', e.target.value)} />
              </div>
            </div>
        ...
        <div className="bg-background rounded-xl border border-border overflow-hidden">
          <div className="px-5 py-4 border-b border-border">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Columns: <code className="bg-muted px-1 rounded">name, email, roll_number, department, semester</code>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    <thead><tr className="border-b border-border">
                      {['Name', 'Email', 'Roll No', 'Dept', 'Sem'].map(h => (
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {csvPreview.slice(0, PAGE_SIZE).map((r, i) => (
                        <tr key={i} className="hover:bg-muted">
                          ...
                          <td className="px-4 py-2.5 font-mono text-muted-foreground">{r.roll_number}</td>
                          <td className="px-4 py-2.5 text-muted-foreground">{r.department}</td>
                          <td className="px-4 py-2.5 text-muted-foreground">{r.semester}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      <div className="bg-background rounded-xl border border-border overflow-hidden">
        <div className="px-5 py-3.5 border-b border-border flex items-center justify-between gap-3">
          <p className="text-sm font-semibold text-foreground">My Students <span className="text-muted-foreground font-normal">({total})</span></p>
          ...
          <>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-50">
                  {students.map(s => (
                    <tr key={s._id} className="hover:bg-muted transition-colors">
                      {editId === s._id ? (
                        <>
                          ...
                          <td className="px-4 py-2"><Input type="number" className="h-7 text-xs border-border w-14" value={editData.semester} onChange={e => setEditData(p => ({ ...p, semester: e.target.value }))} /></td>
                        ...
                        <>
                          ...
                          <td className="px-4 py-2.5 text-muted-foreground">{s.semester}</td>
                          <td className="px-4 py-2.5">
                            <div className="flex items-center gap-2">
                              <button onClick={() => { setEditId(s._id); setEditData({ name: s.name, department: s.department, semester: s.semester }) }}
